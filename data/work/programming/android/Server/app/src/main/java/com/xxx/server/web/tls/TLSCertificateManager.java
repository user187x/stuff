...
public class TLSCertificateManager {
  ...
  public void generateAndStoreCertificate() throws Exception {
    ...
    X509Certificate certificate = new JcaX509CertificateConverter().setProvider(
        BouncyCastleProvider.PROVIDER_NAME).getCertificate(certBuilder.build(contentSigner));

    // Store the key and certificate in a BKS (Bouncy Castle KeyStore)
    KeyStore keyStore = KeyStore.getInstance("PKCS12", BouncyCastleProvider.PROVIDER_NAME);
    keyStore.load(null, null);
    keyStore.setKeyEntry(KEY_ALIAS, privateKey, KEYSTORE_PASSWORD.toCharArray(),
