...
public final class PlayerHolder {
    ...
    public synchronized ExoPlayer getOrCreate(Context context) {
        if (player == null) {
            ...
            if (fmp4SourceFactory == null) {
                ...
                FLog.d(TAG, "Created SeekableFragmentedMp4MediaSourceFactory for VLC-like fMP4 seeking");
            }
            ...
            try {
                ...
                FLog.i(TAG, "AudioManager state: MUSIC=" + musicVol + "/" + musicMax + 
                ...
                FLog.i(TAG, "Manual audio focus request result: " + (result == AudioManager.AUDIOFOCUS_REQUEST_GRANTED ? "GRANTED" : "FAILED"));
            } catch (Exception e) {
                FLog.w(TAG, "Could not log AudioManager state", e);
            }
            
            FLog.d(TAG, "ExoPlayer created with audio attributes, volume=1.0, audioFocus=false");
            ...
            player.addAnalyticsListener(new AnalyticsListener() {
                ...
                public void onAudioEnabled(@NonNull EventTime eventTime, @NonNull DecoderCounters decoderCounters) {
                    FLog.i(TAG, "AUDIO ENABLED - decoder initialized");
                }
                
                @Override
                public void onAudioDisabled(@NonNull EventTime eventTime, @NonNull DecoderCounters decoderCounters) {
                    FLog.w(TAG, "AUDIO DISABLED - decoder shut down. decodedBufferCount=" + decoderCounters.renderedOutputBufferCount);
                }
                ...
                public void onAudioDecoderInitialized(@NonNull EventTime eventTime, @NonNull String decoderName, long initializedTimestampMs, long initializationDurationMs) {
                    FLog.i(TAG, "Audio decoder INITIALIZED: " + decoderName + ", initTime=" + initializationDurationMs + "ms");
                }
                ...
                public void onAudioInputFormatChanged(@NonNull EventTime eventTime, @NonNull Format format, @androidx.annotation.Nullable DecoderReuseEvaluation decoderReuseEvaluation) {
                    FLog.i(TAG, "Audio input format: mime=" + format.sampleMimeType + 
                ...
                public void onAudioPositionAdvancing(@NonNull EventTime eventTime, long playoutStartSystemTimeMs) {
                    FLog.d(TAG, "Audio position ADVANCING - playback started at system time: " + playoutStartSystemTimeMs);
                }
                ...
                public void onAudioTrackInitialized(@NonNull EventTime eventTime, @NonNull androidx.media3.exoplayer.audio.AudioSink.AudioTrackConfig audioTrackConfig) {
                    FLog.i(TAG, "AudioTrack INITIALIZED: encoding=" + audioTrackConfig.encoding + 
                ...
                public void onPlaybackStateChanged(@NonNull EventTime eventTime, int state) {
                    ...
                    FLog.i(TAG, "Playback state changed to: " + stateName);
                }
                
                @Override
                public void onIsPlayingChanged(@NonNull EventTime eventTime, boolean isPlaying) {
                    FLog.i(TAG, "isPlaying changed to: " + isPlaying);
                }
            });
            
            FLog.d(TAG, "ExoPlayer created with DEFAULT configuration - NO custom track selection");
        }
        return player;
    }
    ...
    public synchronized void setMediaIfNeeded(Uri uri) {
        if (player == null || uri == null) {
            FLog.w(TAG, "setMediaIfNeeded called with null player or uri. player=" + player + ", uri=" + uri);
            return;
        }
        
        FLog.d(TAG, "=== setMediaIfNeeded START ===");
        FLog.d(TAG, "URI: " + uri);
        FLog.d(TAG, "Current URI: " + currentUri);
        FLog.d(TAG, "Media item count: " + player.getMediaItemCount());
        ...
        FLog.i(TAG, "fMP4 detection: isFmp4=" + isFmp4 + ", scheme=" + uri.getScheme());
        ...
        if (shouldSetMedia) {
            if (isFmp4) {
                FLog.d(TAG, "Setting fMP4 media (always rebuild index for accurate duration)");
            } else {
                FLog.d(TAG, "Setting new media (URI changed or empty)");
            }
            ...
            FLog.d(TAG, "Calling player.prepare()");
            player.prepare();
            
            FLog.d(TAG, "Media set and prepared");
        } else {
            FLog.d(TAG, "Media already set for this URI, skipping");
        }
        
        // Always ensure volume is max
        player.setVolume(1.0f);
        FLog.d(TAG, "Volume set to 1.0, audio session ID: " + player.getAudioSessionId());
        FLog.d(TAG, "=== setMediaIfNeeded END ===");
    }
    ...
    public synchronized void seekToPosition(long positionMs) {
        ...
        FLog.d(TAG, "Seeking: currentPos=" + currentPos + "ms, targetPos=" + positionMs + 
        ...
        if (playbackState == Player.STATE_ENDED) {
            FLog.d(TAG, "Seeking after ENDED state - will restart playback");
        }
        ...
        FLog.d(TAG, "Seeked to " + positionMs + "ms (player reports: " + player.getCurrentPosition() + "ms)");
    }
