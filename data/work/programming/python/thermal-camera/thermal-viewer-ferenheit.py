...
class Slider:
    """Horizontal drag slider styled to match the button rail.
    Drag left to decrease, right to increase; ticks give it a
    dial-like feel. Reads/writes its value through get/set callables
    so keyboard shortcuts and the slider stay in sync."""
    PAD = 18                                   # horizontal knob inset
...
class ThermalApp:
    def __init__(self, device=None, headless=False):
        ...
        self.sld_blur   = Slider("BLUR",
                                 get=lambda: self.rad,
                                 setter=self.set_blur,
                                 vmin=0, vmax=20)
    ...
    def act_blur_up(self):        # keyboard a
        self.set_blur(self.rad + 1)

    def act_blur_dn(self):        # keyboard z
        self.set_blur(self.rad - 1)
    ...
    def _remap(self, x, y):
        """Window coords -> canvas coords, so taps and drags land
        ...
        try:
            ...
            if ww > 0 and wh > 0 and (ww != CANVAS_W or wh != CANVAS_H):
                return int(x * CANVAS_W / ww), int(y * CANVAS_H / wh)
        except cv2.error:
            pass
        return x, y
    ...
    def on_mouse(self, event, x, y, flags, _):
        ...
        if event == cv2.EVENT_MOUSEMOVE:
            if flags & cv2.EVENT_FLAG_LBUTTON:
                for s in self.sliders:
                    if s.dragging:
                        s.set_from_x(x)      # keep tracking even off-rect
            return
    ...
    def tstr(self, c, dec=1):
        """Formatted temperature in the current unit (no suffix)."""
    ...
    def compose(self, imdata, thdata):
        ...
        outlined_text(heat, f"{self.tstr(m['temp'])} {self.unit}",
                      (cx + 12, cy - 12), 0.45, ACCENT)
        ...
        if m["maxtemp"] > m["avgtemp"] + self.threshold:
            ...
            outlined_text(heat, f"{self.tstr(m['maxtemp'])} {self.unit}",
                          (p[0] + 10, p[1] + 5), 0.45)
        ...
        if m["mintemp"] < m["avgtemp"] - self.threshold:
            ...
            outlined_text(heat, f"{self.tstr(m['mintemp'])} {self.unit}",
                          (p[0] + 10, p[1] + 5), 0.45)
    ...
    def draw_scale_bar(self, canvas, temp_range, oy, vh):
        ...
        text(canvas, f"{self.tstr(lo, 0)}{self.unit}", (bx - 4, by + bh + 16),
             0.42, ACCENT2)
    ...
    def draw_status(self, canvas, m, temps, ox, oy):
        ...
        u = self.unit
        text(canvas, f"CTR {self.tstr(m['temp']):>5}{u}", (ox + 16, oy + 26),
             0.55, ACCENT, 1)
        text(canvas, f"MAX {self.tstr(m['maxtemp']):>5}", (ox + 150, oy + 26),
             0.5, (255, 255, 255))
        text(canvas, f"MIN {self.tstr(m['mintemp']):>5}", (ox + 260, oy + 26),
             0.5, ACCENT2)
        text(canvas, f"AVG {self.tstr(m['avgtemp']):>5}", (ox + 370, oy + 26),
             0.5, TEXT_DIM)
