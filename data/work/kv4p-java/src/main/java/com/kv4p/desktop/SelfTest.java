...
/**
 * Headless validation of the Java port against the firmware's wire format.
 * Emulates the ESP32 side using the exact packed-struct layouts from protocol.h.
 */
public final class SelfTest {
    ...
    static void testStructSizes() {
        check("sizeof(HostDesiredState) == 22", HostDesiredState.SIZE == 22
    ...
    static void testKissEscaping() {
        ...
        var decoder = new KissDecoder(new KissDecoder.Listener() {
            @Override public void onAx25(byte[] ax25) {}
            @Override public void onVendorCommand(int cmd, byte[] p) {
        ...
        check("KISS escape round-trip", received.size() == 1 && java.util.Arrays.equals(received.get(0), payload));
    }
    ...
    static void testVendorFrameLayoutMatchesReadmeExample() {
        ...
        byte[] frame = KissEncoder.vendorFrame(COMMAND_DEBUG_INFO, "Error".getBytes(StandardCharsets.US_ASCII));
        byte[] expected = {(byte) 0xC0, 0x06, 'K', 'V', '4', 'P', 0x01, 0x01, 'E', 'r', 'r', 'o', 'r', (byte) 0xC0};
    ...
    static void testBootBannerNoiseTolerance() {
        ...
        var decoder = new KissDecoder(new KissDecoder.Listener() {
            @Override public void onAx25(byte[] ax25) {}
            @Override public void onVendorCommand(int cmd, byte[] p) { got[0] = cmd; }
        });
        decoder.feed("===== kv4p serial output =====\r\nThis port will emit binary data...\r\n"
    ...
    static void testHelloDecodeFromEmulatedFirmware() {
        ...
        check("HELLO.deviceState.freq_rx == 146.52", Math.abs(h.deviceState().freqRx() - 146.52f) < 1e-4);
    ...
    static void testDesiredStateEncodeDecodedByEmulatedFirmware() {
        var desired = new HostDesiredState(
                7, -1,
                HOST_STATE_RADIO_CONFIG_VALID | HOST_STATE_ENABLE_STATUS_REPORTS
                        | HOST_STATE_RX_AUDIO_OPEN | HOST_STATE_HIGH_POWER | HOST_STATE_TX_ALLOWED,
                BW_25K, 146.52f, 147.345f, 13, 3, 0);
        ...
        check("firmware sees 22-byte HostDesiredState", structBytes != null && structBytes.length == 22);
        ...
        check("flags carry TX_ALLOWED | STATUS_REPORTS",
    ...
    static void testAdpcmBlockGeometry() {
        ...
        check("BLOCK_SAMPLES == AUDIO_FRAME_SAMPLES (249)", ImaAdpcm.BLOCK_SAMPLES == AUDIO_FRAME_SAMPLES);
    }
    ...
    private static byte[] emulateHelloPayload() {
        ...
        b.putInt(-1);                    // memoryId
        b.putShort((short) (HOST_STATE_RADIO_CONFIG_VALID | HOST_STATE_HIGH_POWER | HOST_STATE_RSSI_ENABLED));
        b.put((byte) BW_25K);
    ...
    private static byte[] firmwareSideExtractVendorPayload(byte[] wire) {
        ...
        if (frame.get(1) != 'K' || frame.get(2) != 'V' || frame.get(3) != '4' || frame.get(4) != 'P') return null;
