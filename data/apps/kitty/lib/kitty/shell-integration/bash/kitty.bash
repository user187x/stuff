  ...
  if builtin shopt -q login_shell; then
   if [[ "$kitty_bash_inject" != *"no-profile"* ]]; then
    ...
    for _ksi_i in "$HOME/.bash_profile" "$HOME/.bash_login" "$HOME/.profile"; do
     _ksi_sourceable "$_ksi_i" && {
      builtin source "$_ksi_i"
      break
     }
    done
   fi
...
_ksi_main() {
 ...
 if [ -n "${KITTY_IS_CLONE_LAUNCH}" ]; then
  ...
  if _ksi_s_is_ok "conda" && [ -n "${CONDA_DEFAULT_ENV}" ] && builtin command -v conda >/dev/null 2>/dev/null && [ "${CONDA_DEFAULT_ENV}" != "$orig_conda_env" ]; then
   sourced="y"
   conda activate "${CONDA_DEFAULT_ENV}"
  fi
  if _ksi_s_is_ok "env_var" && [[ -n "${KITTY_CLONE_SOURCE_CODE}" ]]; then
   sourced="y"
   builtin eval "${KITTY_CLONE_SOURCE_CODE}"
  fi
  if _ksi_s_is_ok "path" && [[ -r "${KITTY_CLONE_SOURCE_PATH}" ]]; then
   sourced="y"
