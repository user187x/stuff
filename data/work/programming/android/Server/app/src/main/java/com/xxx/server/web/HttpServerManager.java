...
public class HttpServerManager {
  ...
  private final Set<ServerWebSocket> connectedClients = Collections.newSetFromMap(
  ...
  public HttpServerManager(Context context, Consumer<String> logger,
      Consumer<HttpServerRequest> requestNotifier,
  ...
  private void setupRouter() {
    ...
    // Camera streaming: give the manager the Vert.x context so it can push frames.
    ...
    // ---- Camera live-view routes (registered before the catch-all) ----
  ...
  private void logRequest(HttpServerRequest request, String clientIp, int statusCode,
      long responseTime) {
  ...
  private void handleWebSocketConnection(final ServerWebSocket ws) {
    ...
    ws.textMessageHandler(msg -> {
      ...
      if (msg.startsWith("SIGNAL_TYPING:")) {
        ...
        statsManager.onWebSocketData(msg.length(), 0);
  ...
  public void broadcastChatMessage(String sender, String message) {
    ...
    for (ServerWebSocket client : connectedClients) {
      ...
      statsManager.onWebSocketData(0, formatted.length());
    }
  ...
  private void handleStatusRequest(io.vertx.ext.web.RoutingContext context) {
    String timestamp = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(
        new Date());
  ...
  private void handleRequest(io.vertx.ext.web.RoutingContext context) {
    ...
    logger.accept(
        "REQ: " + path + " (Root: " + (rootFolder.isEmpty() ? "NOT_SET" : "ACTIVE") + ")");

    if (rootFolder == null || rootFolder.isEmpty()) {
      context.response().setStatusCode(404)
          .end("ERROR: Root directory not configured in app settings.");
      return;
    }
  ...
  private void handleDirectoryListing(io.vertx.ext.web.RoutingContext context,
      DocumentFile directory, String path) {
  ...
  private void handleDirectoryListingFile(io.vertx.ext.web.RoutingContext context,
      java.io.File directory, String path) {
    if (!renderFolderContent) {
      context.response().setStatusCode(403).end("Directory listing disabled");
      return;
    }
  ...
  private void generateDirectoryListingHtml(io.vertx.ext.web.RoutingContext context,
      DocumentFile directory, String path) {
    ...
    if (!path.equals("/")) {
      ...
      html.append("<li><a href=\"").append(parentPath)
          .append("\"><span class=\"icon\">&#11013;</span> .. (Parent Directory)</a></li>");
    }

    DocumentFile[] files = directory.listFiles();
    if (files == null || files.length == 0) {
      html.append(
          "<li style=\"color:#888;justify-content:center;\">NO_FILES_FOUND_IN_DIRECTORY</li>");
    } else {
      for (DocumentFile file : files) {
        ...
        html.append("<li>");
        html.append("<a href=\"").append(filePath).append("\"><span class=\"icon\">").append(icon)
            .append("</span> ").append(fileName).append("</a>");
        if (!isDir) {
          html.append("<div class=\"actions\"><a href=\"").append(filePath)
              .append("\" download class=\"btn btn-download\">DOWNLOAD</a></div>");
        }
        html.append("</li>");
      }
    }
    ...
    if (allowUploads) {
      ...
      html.append("<form action=\"/upload?dir=").append(path)
  ...
  private void generateDirectoryListingHtmlFile(io.vertx.ext.web.RoutingContext context,
      java.io.File directory, String path) {
    StringBuilder html = new StringBuilder();
    html.append(getHtmlHeader("File Explorer - " + path));
    html.append("<h1>FILE_EXPLORER_v1.0</h1>");
  ...
  private void serveDocumentFile(io.vertx.ext.web.RoutingContext context, DocumentFile file) {
    try {
      ...
      logger.accept(
          "FILE: Accessing " + file.getName() + " [" + getClientIp(context.request()) + "]");
      ...
      while ((bytesRead = inputStream.read(buffer)) != -1) {
        context.response()
            .write(io.vertx.core.buffer.Buffer.buffer(java.util.Arrays.copyOf(buffer, bytesRead)));
      }
  ...
  private void saveToDocumentTree(io.vertx.ext.web.RoutingContext context, String path,
  ...
  private void saveToFileSystem(io.vertx.ext.web.RoutingContext context, String path,
      String fileName, String tempFile) throws IOException {
    java.io.File rootDir = new java.io.File(rootFolder);
