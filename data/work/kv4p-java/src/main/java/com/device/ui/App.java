...
import com.device.manager.DeviceManager;
import com.device.audio.RxAudioPlayer;
import com.device.audio.TxAudioCapture;
...
public final class App extends JFrame {
  ...
  private final JCheckBox txAllowedCheck = new JCheckBox("TX allowed (unlock transmit)");
  ...
  private JPanel buildLivePanel() {
    ...
    panel.setBorder(new TitledBorder("Live"));
  ...
  private void wireActions() {
    ...
    filterPreCheck.addActionListener(e -> ifConnected(c -> c.setFlag(HOST_STATE_FILTER_PRE, filterPreCheck.isSelected())));
    filterHighCheck.addActionListener(e -> ifConnected(c -> c.setFlag(HOST_STATE_FILTER_HIGH, filterHighCheck.isSelected())));
    filterLowCheck.addActionListener(e -> ifConnected(c -> c.setFlag(HOST_STATE_FILTER_LOW, filterLowCheck.isSelected())));
  ...
  private void connect() {
    ...
    try {
      ...
      client = DeviceManager.connect(link);
      ...
      // Start serial I/O only after the listener and audio pipeline are fully wired,
      // so the one-shot HELLO handshake cannot be dispatched before anyone is listening.
      client.start();
  ...
  private void disconnect(String reason) {
    if (txCapture != null) { txCapture.close(); txCapture = null; }
  ...
  private void tune() {
    ifConnected(c -> {
      try {
        ...
        JOptionPane.showMessageDialog(this, "Invalid frequency.", "kv4p", JOptionPane.WARNING_MESSAGE);
      }
    });
  }
  ...
  private void ifConnected(java.util.function.Consumer<DeviceManager> action) {
    DeviceManager c = client;
    if (c != null) action.accept(c);
  }

  private void setControlsEnabled(boolean enabled) {
    for (JComponent comp : new JComponent[]{freqRxField, freqTxField, bwCombo, squelchSpinner,
        ctcssTxSpinner, ctcssRxSpinner, tuneBtn, highPowerCheck, filterPreCheck,
        filterHighCheck, filterLowCheck, txAllowedCheck, rxAudioCheck, pttButton}) {
  ...
  // ---------------- Client events ----------------
  private final class UiListener implements DeviceManager.Listener {
    @Override
    ...
    public void onDeviceState(DeviceState state) {
      SwingUtilities.invokeLater(() -> {
        modeLabel.setText(switch (state.mode()) {
