...
public class ThermalCamera {
 ...
 // The canvas is composed at a fixed internal resolution and then scaled
 // to whatever size the window is - see CanvasPanel below.
 ...
 // Sensor dimensions - derived per frame from buffer + layout + stride.
 private int srcW = 256;
 private int srcH = 192;

 // Layout modes. LINE-INT = image on even lines, thermal on odd lines
 // (the comb/striping artifacts you saw are the signature of this packing).
 private static final String[] LAYOUT_NAMES = {"STACKED", "SIDE-BY-SIDE", "LINE-INT"};
 private int layoutMode = 0;

 // Stride tuner: reinterpret the raw buffer at a chosen line width.
 // 0 = AUTO (use whatever V4L2 reports). Cycle with [ and ].
 ...
 public void run(int deviceIndex) {
  ...
  System.out.println("Negotiated FOURCC: "
  ...
  System.out.println("Keys: l=layout  [ ]=stride  r/e=rotate  m=palette  p=snapshot  q=quit");

  // RESIZABLE UI: CanvasPanel scales the composed frame to the window,
  // preserving aspect ratio, and maps mouse coords back to canvas space.
  ...
  while (running) {
   ...
   // 1) Optional stride reinterpretation of the whole buffer.
   Mat buf = rawFrame;
   ...
   // 2) Split into image + thermal per the selected layout.
   Mat imdata, thdata;
   switch (layoutMode) {
    case 1: { // SIDE-BY-SIDE
     srcW = buf.cols() / 2;
    ...
    case 2: { // LINE-INTERLEAVED: even rows image, odd rows thermal
     srcW = buf.cols();
     srcH = buf.rows() / 2;
   ...
   panel.setImage(img); // repaints itself; scaling happens in paintComponent
 ...
 /**
  * RESIZABLE UI: draws the fixed-resolution canvas scaled to fit the
  * current window size (letterboxed to preserve aspect), and converts
  * window coordinates back to canvas coordinates for the controls.
  */
 private static class CanvasPanel extends JPanel {
  ...
  CanvasPanel() { setBackground(new Color(24, 18, 14)); setFocusable(true); }
 ...
 private void actStride(int dir) {
  ...
  syncLayoutLabel();
  System.out.println("Stride: " + (STRIDES[strideIdx] == 0 ? "AUTO" : STRIDES[strideIdx]));
 }
