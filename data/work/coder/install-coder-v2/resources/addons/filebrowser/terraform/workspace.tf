...
variable "ca_secret_name" {
  type        = string
  description = "The name of the Kubernetes secret containing the CA cert"
  default     = "coder-xxx-ca"
}
...
resource "coder_agent" "main" {
  ...
  startup_script = <<-EOT
    ...
    filebrowser_url="/@$${data.coder_workspace_owner.me.name}/$${data.coder_workspace.me.name}.main/apps/file-browser"

    # Pass the proxied base URL to Filebrowser so it loads static assets properly
    filebrowser_url="/@${data.coder_workspace_owner.me.name}/${data.coder_workspace.me.name}.main/apps/file-browser"

    # Execute the filebrowser binary
    /opt/filebrowser -r /home/coder -p 13339 -a 0.0.0.0 --noauth --baseurl "$filebrowser_url" >/tmp/filebrowser.log 2>&1 &
...
resource "kubernetes_deployment_v1" "main" {
  ...
  spec {
    ...
    template {
      ...
      spec {
        container {
          name              = "filebrowser-sidecar"
          image             = "coder-filebrowser:latest" # <- sidecar image holding filebrowser binary
          image_pull_policy = "IfNotPresent"             # <- forces Minikube to use local image
          
          # Root CA setup so Coder can connect securely to your coder.xxx domain
          command           = ["sh", "-c", "exec sh -c \"$CODER_INIT_SCRIPT\""]
